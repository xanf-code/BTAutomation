...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.label_1 = customtkinter.CTkLabel(master=self.frame_left,
                                              text="Select Environment")  # font name and size in px
        self.label_1.grid(row=1, column=0, pady=10, padx=10)

        self.button_1 = customtkinter.CTkButton(master=self.frame_left,
                                                text="QaRFT",
                                                fg_color=("gray75", "gray30"),  # <- custom tuple-color
                                                command=self.pw_auto)
        ...
        self.label_info_1 = customtkinter.CTkTextbox(master=self.frame_info, width=250)
    ...
    def eta(self):
        #self.label_info_1.configure(text = "Trying to run whatever shit its going to bere")
        for i in range(10):
            self.progressbar.set(i/10)
            updatetext="Trying to run whatever shit its going to b" + str(i/10)+ " %" +"\n"
            
            self.label_info_1.insert(0.0,updatetext)
            self.update()
            time.sleep(1)
    ...
    def etadem(self):
        ...
        textup=str(self.progress * 100)+" %"
        self.label_info_1.configure(text = textup)
        if(self.progress < 1):
            self.after(20,self.eta)
            
    def pw_auto(self):
        with sync_playwright() as playwright: 
            self.progress = 2
            run(playwright,self)
            # pw_automation.main(playwright)
        # print("Hello")
